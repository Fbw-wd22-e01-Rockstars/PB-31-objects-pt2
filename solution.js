...
function countLetters(string){
  /*   let result = {}
    ...
    let result = string.split("").reduce(function(acc,char){
        if(acc[char]){
            acc[char]++;
        }else{
            acc[char]= 1;
...
/* const countLetters = string.split("").reduce((acc,char)=> acc[char]?acc[char]++ : 1,{} ) */
...
/* for(const language in programming.languages){
